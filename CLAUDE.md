...
*Last updated: iOS Dashboard redesign (8-tile Controller card, Pumps Overview with MiniTankView, Today's Dosing card with hourly heatmap, DurationStepper/DirectionToggle/SpeedStepper manual run controls); StepperSelectionChip `expand` param + ScrollView pump-name rows; StepperFont sizes raised (min 11pt), mutedForeground brightened (slate-400/slate-600); StepperInputShell height-stacking pitfall documented; iOS native app (SwiftUI, @Observable, StepperTokens, haptics, RealtimeConnection WebSocket); shared API reference; BLE provisioning; Board Configuration presets (Fysetc E4 v1.0); Backup & Restore; font scale selector; pump safety limits; TMC2209 health; language/i18n deferred.*
