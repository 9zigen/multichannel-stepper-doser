...
const ScheduleForm = ({pump, success}: ScheduleFormProps): React.ReactElement => {
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        if (await updatePumps(data, false)) {
                        ...
                        modeActual !== SCHEDULE_MODE.OFF? (
                            <div className="flex flex-row gap-4 mb-4 justify-between">
                                <div className={modeActual === SCHEDULE_MODE.PERIODIC? 'w-[50%]' : 'w-full'}>
                                    ...
                                    />
                                    {errors.schedule?.speed && <p role="alert" className="text-sm text-red-600">{errors.schedule?.speed?.message}</p>}
                                </div>
                                    ...
                                    modeActual === SCHEDULE_MODE.PERIODIC? (
                                        <React.Fragment>
                                            <div className="w-[50%]">
                                                ...
                                                />
                                                {errors.schedule?.volume && <p role="alert" className="text-sm text-red-600">{errors.schedule?.volume?.message}</p>}
                                            </div>
                                            ...
                                            {/*        min="0.1"*/}
                                            {/*        step="0.1"*/}
                                            {/*        {...register("schedule.time", { valueAsNumber: true })}*/}
                                            {/*    />*/}
                                            {/*    {errors.schedule?.time && <p role="alert" className="text-sm text-red-600">{errors.schedule?.time?.message}</p>}*/}
                                            {/*</div>*/}
                                        </React.Fragment>) : null
                                }
                            </div>) : null
