...
import { PUMP_MODE, PumpCalibrationState, PumpState } from "@/lib/api.ts";
...
const FormSchema = z
    .object({
        ...
        pump_mode: z.union([z.literal(0), z.literal(1), z.literal(2)]),
...
const PumpForm = ({pump, success}: PumpFormProps): React.ReactElement => {
    const { id, state, name, direction, tank_full_vol, tank_current_vol, tank_concentration_total, tank_concentration_active, pump_mode, calibration } = pump;
    const updatePumps = useAppStore((state: AppStoreState) => state.updatePumps);

    const { control, register, handleSubmit, formState: { errors }, setError, clearErrors, setValue, watch } = useForm<FormData>({
    ...
    const removeCalibration = (index: number) => {
        ...
        updatePumps({
            ...pump,
            calibration: newCalibrations
        }).then(
            () => {
                toast.success("Pump calibration updated. Please save the pump settings to apply the changes.")
            },
            () => {
                toast.error("Pump calibration not updated.")
            }
        )
    }

    const addCalibration = (cal: PumpCalibrationState) => {
    ...
    return (
        <React.Fragment>
            <ScrollArea className="h-[500px] xl:h-[380px] w-full p-2">
                <div className="flex flex-col xl:flex-row xl:gap-6 p-2">
                    <form id="pump-form" className="flex flex-col xl:w-[50%]" onSubmit={handleSubmit(onSubmit, (errors) => console.log(errors))}>
                        ...
                        <div className="flex flex-row gap-4 mb-6 justify-between">
                            <div className="w-[50%]">
                                ...
                                <Input
                                    type="number"
                                    placeholder="900"
                                    {...register("tank_full_vol", { valueAsNumber: true })}
                                />
                                {errors.tank_full_vol && <p role="alert" className="text-sm text-red-600">{errors.tank_full_vol?.message}</p>}
                            </div>
                            ...
                            <div className="w-[50%]">
                                ...
                                <Input
                                    type="number"
                                    placeholder="900"
                                    {...register("tank_current_vol", { valueAsNumber: true })}
                                />
                                {errors.tank_current_vol && <p role="alert" className="text-sm text-red-600">{errors.tank_current_vol?.message}</p>}
                            </div>
                        </div>
                        ...
                        <div className="flex flex-row gap-4 mb-6 justify-between">
                            <div className="w-[50%]">
                                ...
                                />
                                {errors.tank_concentration_total && <p role="alert" className="text-sm text-red-600">{errors.tank_concentration_total?.message}</p>}
                            </div>
                            ...
                            <div className="w-[50%]">
                                ...
                                />
                                {errors.tank_concentration_active && <p role="alert" className="text-sm text-red-600">{errors.tank_concentration_active?.message}</p>}
                            </div>
                        </div>
                    </form>
                    ...
                    <div className="flex flex-col xl:w-[50%]">
                        ...
                        {errors.calibration && <p role="alert" className="text-sm text-red-600">{errors.calibration?.message}</p>}
                        {
                            pumpCalibrations.map((item, index) => (
                                <div key={index} className="flex flex-row gap-4 mb-4 bg-gray-100 p-2 rounded-md justify-between">
                                    ...
                                    <div className="w-[100px] flex items-center">
                                        <Button type="button" variant='destructive' className="w-full h-full" onClick={() => removeCalibration(index)}>Delete</Button>
                                    </div>
                                </div>
                            ))
                        }
            ...
            <div className="flex flex-row">
                <Button type="submit" className="w-full" form="pump-form">
                    Save
                </Button>
            </div>
        </React.Fragment>
