...
#define APP_PUMP_HISTORY_VOLUME_SCALE 100U
#define APP_PUMP_HISTORY_VOLUME_MAX_CML UINT32_MAX
#define APP_PUMP_HISTORY_VOLUME_MAX_ML \
    ((double)APP_PUMP_HISTORY_VOLUME_MAX_CML / (double)APP_PUMP_HISTORY_VOLUME_SCALE)
...
typedef struct __attribute__((packed)) {
    uint32_t scheduled_volume_cml;
    uint32_t manual_volume_cml;
    uint16_t total_runtime_s;
    uint8_t flags;
    uint8_t reserved;
} pump_history_hour_t;

static inline double app_pumps_history_volume_cml_to_ml(uint32_t volume_cml)
{
    return (double)volume_cml / (double)APP_PUMP_HISTORY_VOLUME_SCALE;
}
