                               ...
                               int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        {
            network_t * network_config = get_networks_config(active_sta_point + 1);
...
//        xTimerStart( xReconnectTimer, 100 / portTICK_PERIOD_MS);
//        set_led_mode(LED_INDICATE_ERROR, LED_THREE_BLINK, 255);
//      }
        }
...
void vReconnectTimerCallback( TimerHandle_t pxTimer )
{
    ...
    network_t * network_config = get_networks_config(active_sta_point);

    if (network_config->active &&  mode == WIFI_MODE_AP) {
        wifi_init_sta_after_ap(network_config);
    }
    esp_wifi_connect();
    ESP_LOGI(TAG, "timer reconnect to AP");
}
...
static esp_err_t wifi_init_sta(network_t * config)
{
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        network_t * active_wifi_config = get_networks_config(active_sta_point);
...
static esp_err_t connect_wifi(void)
{
    ...
    ESP_LOGI(TAG, "total_sta_point %d", total_sta_point);
...
void initialise_wifi(void *arg)
{
    ...
    /* reconnect timer */
    xReconnectTimer = xTimerCreate( "WiFiStaTimer", ( 10 * 1000 / portTICK_PERIOD_MS), pdFALSE, 0, vReconnectTimerCallback);
